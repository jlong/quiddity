...
`.quiddity/project.md` is a summary of the project's structure, tech stack,
key files, and conventions. It's generated by scanning the repository.

`/q-scan-project` is responsible for generating this file. It runs first in
the setup flow so that subsequent skills have project context.
...
/q-setup
  │
  ├─ /q-scan-project
  │     └─ writes .quiddity/project.md
  │
  ├─ /q-which-process
  │     ├─ reads .quiddity/project.md for context
...
### How /q-scan-project works

1. Scans the project for key files, documentation, configuration, and conventions
2. Notes the tech stack, project structure, and CI/CD setup
3. Asks the user if anything important is missing
4. Writes `.quiddity/project.md`

### How /q-which-process works

1. Reads `.quiddity/project.md` for context (if it exists)
2. Asks the user if they have an existing process document to share
...
1. Reads `.quiddity/project.md` and `.quiddity/process.md` for context (if they exist)
...
project.md, process.md, and tools.json and won't re-interview.

## File locations

| Path | Purpose |
|---|---|
| `.quiddity/project.md` | Project structure summary (generated) |
